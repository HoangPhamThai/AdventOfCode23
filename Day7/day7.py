...
class Hand:
    ...
    def __repr__(self) -> str:
        return f"{self.cards} - {self.bid} - {self.type} - {self.encodeCards} - {self.rank}"
...
class ListHand:
    ...
    def __repr__(self) -> str:
        return f"{self.listHand} - {self.type}"
    ...
    def getLastRank(self):
        if (len(self.listHand) == 0): return 0
...
def process(data: list[Hand]):
    ...
    total = list5Kind.getTotalWinnings() + list4Kind.getTotalWinnings() + listFullHouse.getTotalWinnings() + list3Kind.getTotalWinnings() + list2Pair.getTotalWinnings() + list1Pair.getTotalWinnings() + listHighCard.getTotalWinnings()
    print(total)
